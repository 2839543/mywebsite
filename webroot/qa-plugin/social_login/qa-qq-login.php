...
class QQ_Login extends Connect{
	...
	public function check_login() {
		...
		if (is_array($user) && $user['ret'] == 0 && strlen($openid)) {
			qa_log_in_external_user($this->id, $openid, array (
				...
				'avatar' =>   strlen(@ $user['figureurl']) && !qa_db_user_login_find($this->id, $openid) ? qa_retrieve_url($user['figureurl']) : null,
			));
		}
	}
	...
	private function callback(){
		if(isset($_REQUEST['state']) && isset($_SESSION['state']) && $_REQUEST['state'] == $_SESSION['state']){//csrf
			$token_url = "{$this->api}/oauth2.0/token?grant_type=authorization_code&"."client_id=".$this->key."&redirect_uri=".urlencode($_SESSION['tourl'])
